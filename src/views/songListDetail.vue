<template>
    <div class="listDetail" v-if="palyDetail">
        ...
        <div class="playListMain">
            ...
            <ul class="song-list">
                <div class="playAll" @click="addAll" >
                    <i class="iconfont icon-weibiaoti518"></i>
                    <div>播放全部<span>{{palyDetail.tracks.length}}</span></div> 
                    <div class="info-time"><span>{{getLocalTime() | uptime}} 更新</span></div>
                </div>
                <li v-for="(item,index) in palyDetail.tracks" :key="index" :class="[{'active-paly': item.id == id}]">
                    <a>
                        <div class="Ranking">
                        ...
                        <div class="icon">
                            <i :class="['download','iconfont', {'icon-weibiaoti519':item.id == id},{'icon-weibiaoti518': item.id !== id}]" @click="getSongID(item.id)"></i>
...
export default {
    ...
    methods:{
        ...
        addAll(){
            this.downUp =true;
            this.$emit('addAll',this.palyDetail.tracks)
            setTimeout(()=>{
                this.downUp = false
            },500)
        }
